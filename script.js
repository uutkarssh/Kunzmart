/* ============================================================
   KUNZ MART — Interactions v4.0
   Mobile nav: horizontal scroll bar (km-mobile-nav)
   Hamburger removed — replaced by scroll nav on mobile
   ============================================================ */
    ...
    if (!o.visible) {
      ...
      if (!cfg.notice || !cfg.notice.visible) {
        ...
        if (hero) {
          ...
          hero.style.paddingTop = isMobile
            ? 'calc(var(--nav-h) + var(--mobile-nav-h))'
            : 'var(--nav-h)';
        }
      }
    } else {
...
/* ── Mobile Nav — active state ─────────────────────────────
   Marks the correct km-mobile-nav-item as active on every page.
   Runs immediately (site-config.js already loaded synchronously).
────────────────────────────────────────────────────────────── */
(function () {
  const page = window.location.pathname.split('/').pop() || 'index.html';
  document.querySelectorAll('.km-mobile-nav-item').forEach(item => {
    const itemPage = item.getAttribute('data-page') || item.getAttribute('href');
    const isActive = itemPage === page || (page === '' && itemPage === 'index.html');
    if (isActive) {
      item.classList.add('nav-active');
      /* Scroll the active item into view within the track */
      const track = document.getElementById('kmMobileNavTrack');
      if (track) {
        requestAnimationFrame(() => {
          const offset = item.offsetLeft - track.offsetLeft;
          const center = offset - (track.clientWidth / 2) + (item.clientWidth / 2);
...
(function () {
  ...
  function lerp() {
    ...
    rafId = Math.abs(mx - rx) > 0.1 || Math.abs(my - ry) > 0.1
      ? requestAnimationFrame(lerp) : null;
  }
  ...
  document.querySelectorAll('a,button,.dept-card,.prize-card,.gallery-item,.gallery-full-item,.branch-card,.dept-section').forEach(el => {
    el.addEventListener('mouseenter', () => {
      dot.style.transform  = `translate(calc(-50% + ${mx}px), calc(-50% + ${my}px)) scale(2.5)`;
      ring.style.width = ring.style.height = '56px'; ring.style.opacity = '.35';
    }, { passive: true });
    el.addEventListener('mouseleave', () => {
      dot.style.transform  = `translate(calc(-50% + ${mx}px), calc(-50% + ${my}px)) scale(1)`;
...
/* ── Hamburger removed — replaced by km-mobile-nav (horizontal scroll bar) ── */
...
(function () {
  ...
  /* Touch / swipe — all passive for no scroll blocking */
...
/* ── Hero orb parallax — desktop only ──────────────────────── */
(function () {
  ...
  window.addEventListener('scroll', () => {
    if (!ticking) {
      requestAnimationFrame(() => {
        ...
        orbs.forEach((orb, i) => { orb.style.transform = `translateY(${y * (i + 1) * 0.08}px)`; });
        ticking = false;
      });
      ticking = true;
    }
  ...
  document.querySelectorAll('a[href^="#"]').forEach(link => {
    link.addEventListener('click', e => {
      ...
      if (target) {
        e.preventDefault();
        const navbar    = document.querySelector('.navbar');
        const mobileNav = document.querySelector('.km-mobile-nav');
        let offset = navbar ? navbar.offsetHeight : 76;
        /* On mobile, also account for the horizontal scroll nav bar */
